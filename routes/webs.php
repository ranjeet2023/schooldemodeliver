...
use App\Http\Controllers\UserGuardian\HomeController;
use App\Http\Controllers\UserStaff\HomeController;
use App\Http\Controllers\Front\PostalExchangeController;
use App\Http\Controllers\Front\PostalExchangeTypeController;
use App\Http\Controllers\Front\VisitorController;
use App\Http\Controllers\Front\VisitorPurposeController;
...
use App\Http\Controllers\Account\Fees\FeesBaseController;
use App\Http\Controllers\Account\Fees\FeesHeadController;
use App\Http\Controllers\Account\Fees\FeesMasterController;
use App\Http\Controllers\Account\Fees\FeesCollectionController;
use App\Http\Controllers\Account\Fees\FeesCollectionController;
use App\Http\Controllers\Account\Fees\OnlinePaymentController;
use App\Http\Controllers\Account\Fees\Payment\PayPalController;
use App\Http\Controllers\Account\Fees\Payment\StripePaymentController;
use App\Http\Controllers\Account\Fees\Payment\InstamojoPaymentController;
use App\Http\Controllers\Account\Fees\Payment\PayumoneyPaymentController;
use App\Http\Controllers\Account\Fees\Payment\PayStackPaymentController;
use App\Http\Controllers\Account\Fees\Payment\RozorPayPaymentController;
use App\Http\Controllers\Payroll\PayrollBaseController;
use App\Http\Controllers\Payroll\PayrollHeadController;
use App\Http\Controllers\Payroll\PayrollMasterController;
use App\Http\Controllers\Payroll\SalaryPayController;
use App\Http\Controllers\Transaction\AccountGroupController;
use App\Http\Controllers\Transaction\TransactionHeadController;
...
use App\Http\Controllers\Report\CashBookReportController;
use App\Http\Controllers\Report\FeeCollectionReportController;
use App\Http\Controllers\Report\FeeCollectionHeadReportController;
use App\Http\Controllers\Report\OnlineFeePaymentReportController;
use App\Http\Controllers\Report\BalanceFeeReportController;
use App\Http\Controllers\Library\LibraryBaseController;
...
/* Group Middleware for Role Routes */
Route::middleware(['ability:super-admin'])->group(function () {
    Route::get('role', [RoleController::class, 'index'])->name('role');
    Route::middleware(['role-add'])->group(function () {
        Route::get('role/add', [RoleController::class, 'create'])->name('role.add');
        Route::post('role/store', [RoleController::class, 'store'])->name('role.store');
    });
    Route::middleware(['role-edit'])->group(function () {
    ...
    Route::middleware(['role-view'])->group(function () {
        Route::get('role/{id}/view', [RoleController::class, 'show'])->name('role.view');
    });
...
/* Group Middleware for User Routes */

Route::middleware(['ability:super-admin'])->group(function () {
    Route::get('user', [UserController::class, 'index'])->name('user');
    Route::middleware(['user-add'])->group(function () {
    ...
    Route::post('user/{id}/password-change', [UserController::class, 'changePassword'])->name('user.password-change');
    ...
    Route::middleware(['user-in-active'])->group(function () {
        Route::get('user/{id}/in-active', [UserController::class, 'inActive'])->name('user.in-active');
    });
    Route::middleware(['user-bulk-action'])->group(function () {
        Route::post('user/bulk-action', [UserController::class, 'bulkAction'])->name('user.bulk-action');
    });
});


Route::prefix('user-student')->as('user-student.')->middleware(['role:student', 'permission:student-dashboard'])->namespace('UserStudent')->group(function () {
        Route::get('', [HomeController::class, 'index'])->name('');
        Route::get('profile', [HomeController::class, 'profile'])->name('profile');
        Route::middleware(['permission:student-profile-edit'])->group(function () {
            Route::get('profile/{id}/edit', [HomeController::class, 'editProfile'])->name('profile.edit');
            Route::post('profile/{id}/update', [HomeController::class, 'updateProfile'])->name('profile.update');
            Route::post('{id}/password', [HomeController::class, 'password'])->name('password');
        });
        Route::get('fees', [HomeController::class, 'fees'])->name('fees')->middleware('permission:student-fees');
        Route::get('library', [HomeController::class, 'library'])->name('library')->middleware('permission:student-library');
        Route::get('library/book-list', [HomeController::class, 'bookList'])->name('library.book-list')->middleware('permission:student-library');
        Route::get('library/{id}/request-book', [HomeController::class, 'requestBook'])->name('library.request-book')->middleware('permission:student-library');
        Route::get('attendance', [HomeController::class, 'attendance'])->name('attendance')->middleware('permission:student-attendance');
        Route::get('exams', [HomeController::class, 'exams'])->name('exams')->middleware('permission:student-exam');
        Route::get('exam-schedule/{year}/{month}/{exam}/{faculty}/{semester}', [HomeController::class, 'examSchedule'])->name('exam-schedule')->middleware('permission:student-exam');
        Route::get('exam-admit-card/{year}/{month}/{exam}/{faculty}/{semester}', [HomeController::class, 'admitCard'])->name('exam-admit-card')->middleware('permission:student-exam');
        Route::get('exam-score/{year}/{month}/{exam}/{faculty}/{semester}', [HomeController::class, 'examScore'])->name('exam-score')->middleware('permission:student-exam');
        Route::get('hostel', [HomeController::class, 'hostel'])->name('hostel')->middleware('permission:student-hostel');
        Route::get('transport', [HomeController::class, 'transport'])->name('transport')->middleware('permission:student-transport');
        Route::get('subject', [HomeController::class, 'subject'])->name('subject')->middleware('permission:student-course');
        Route::get('notice', [HomeController::class, 'notice'])->name('notice')->middleware('permission:student-notice');
        Route::get('download', [HomeController::class, 'download'])->name('download')->middleware('permission:student-download');
        Route::get('assignment', [HomeController::class, 'assignment'])->name('assignment')->middleware('permission:student-assignment');
        Route::prefix('assignment/answer')->middleware('permission:student-assignment')->group(function () {
            Route::get('{id}/add', [HomeController::class, 'addAnswer'])->name('assignment.answer.add');
            Route::post('store', [HomeController::class, 'storeAnswer'])->name('assignment.answer.store');
            Route::get('{id}/edit', [HomeController::class, 'editAnswer'])->name('assignment.answer.edit');
            Route::post('{id}/update', [HomeController::class, 'updateAnswer'])->name('assignment.answer.update');
            Route::get('{id}/{answer}/view', [HomeController::class, 'viewAssignmentAnswer'])->name('assignment.answer.view');
        });
       Route::get('meeting', [HomeController::class, 'meeting'])->name('meeting')->middleware('permission:student-meeting');
});
...
Route::prefix('user-guardian')
    ->as('user-guardian.')
    ->middleware(['role:guardian'])
    ->namespace('UserGuardian')
    ->group(function () {
        Route::get('', [HomeController::class, 'index'])->name('');
        Route::middleware(['permission:guardian-dashboard'])->group(function () {
            Route::get('profile', [HomeController::class, 'profile'])->name('profile');
            Route::post('{id}/password', [HomeController::class, 'password'])->name('password');
        });
        // Guardian's student-wise summary routes
        Route::get('students', [HomeController::class, 'students'])->name('students')->middleware('permission:guardian-student-list');
        Route::get('students/{id}/profile', [HomeController::class, 'studentProfile'])->name('students.profile')->middleware('permission:guardian-student-profile');
        Route::get('students/{id}/fees', [HomeController::class, 'fees'])->name('students.fees')->middleware('permission:guardian-student-fees');
        Route::get('students/{id}/library', [HomeController::class, 'library'])->name('students.library')->middleware('permission:guardian-student-library');
        Route::get('students/{id}/attendance', [HomeController::class, 'attendance'])->name('students.attendance')->middleware('permission:guardian-student-attendance');
        Route::get('students/{id}/hostel', [HomeController::class, 'hostel'])->name('students.hostel')->middleware('permission:guardian-student-hostel');
        Route::get('students/{id}/transport', [HomeController::class, 'transport'])->name('students.transport')->middleware('permission:guardian-student-transport');
        Route::get('students/{id}/subject', [HomeController::class, 'subject'])->name('students.subject')->middleware('permission:guardian-student-course');
        Route::get('students/{id}/download', [HomeController::class, 'download'])->name('students.download')->middleware('permission:guardian-student-download');
        Route::get('students/{id}/exams', [HomeController::class, 'exams'])->name('students.exams')->middleware('permission:guardian-student-exam');
        Route::get('students/{id}/exam-schedule/{year}/{month}/{exam}/{faculty}/{semester}', [HomeController::class, 'examSchedule'])->name('students.exam-schedule')->middleware('permission:guardian-student-exam');
        Route::get('students/{id}/exam-admit-card/{year}/{month}/{exam}/{faculty}/{semester}', [HomeController::class, 'admitCard'])->name('students.exam-admit-card')->middleware('permission:guardian-student-exam');
        Route::get('students/{id}/exam-score/{year}/{month}/{exam}/{faculty}/{semester}', [HomeController::class, 'examScore'])->name('students.exam-score')->middleware('permission:guardian-student-exam');
        // Guardian's student-wise assignment-related routes
        Route::get('students/{id}/assignment', [HomeController::class, 'assignment'])->name('students.assignment')->middleware('permission:guardian-student-assignment');
        Route::get('students/{id}/assignment/answer/{assignment}/{answer}/view', [HomeController::class, 'viewAssignmentAnswer'])->name('students.assignment.answer.view')->middleware('permission:guardian-student-assignment');
...
Route::prefix('user-staff')
    ...
    ->group(function () {
        ...
        Route::get('library/book-list', [HomeController::class, 'bookList'])->name('library.book-list');
        Route::get('library/{id}/request-book', [HomeController::class, 'requestBook'])->name('library.request-book');
        Route::get('attendance', [HomeController::class, 'attendance'])->name('attendance');
        Route::get('exam-score', [HomeController::class, 'examScore'])->name('exam-score');
        Route::get('hostel', [HomeController::class, 'hostel'])->name('hostel');
        Route::get('transport', [HomeController::class, 'transport'])->name('transport');
...
// // /*Students Grouping*/        starrteadasd
...
Route::prefix('front')
    ...
    ->group(function () {
    /* Postal Exchange Routes */
    Route::middleware(['ability:super-admin,postal-exchange-index'])->group(function () {
        Route::get('postal-exchange', [PostalExchangeController::class, 'index'])->name('postal-exchange');
        Route::get('postal-exchange/add', [PostalExchangeController::class, 'add'])->name('postal-exchange.add');
        Route::post('postal-exchange/store', [PostalExchangeController::class, 'store'])->name('postal-exchange.store');
        Route::get('postal-exchange/{id}/edit', [PostalExchangeController::class, 'edit'])->name('postal-exchange.edit');
        Route::post('postal-exchange/{id}/update', [PostalExchangeController::class, 'update'])->name('postal-exchange.update');
        Route::get('postal-exchange/{id}/delete', [PostalExchangeController::class, 'delete'])->name('postal-exchange.delete');
        Route::post('postal-exchange/bulk-action', [PostalExchangeController::class, 'bulkAction'])->name('postal-exchange.bulk-action');
    });
    /* Postal Exchange Type Routes */
    Route::middleware(['ability:super-admin,postal-exchange-type-index'])->group(function () {
        Route::get('postal-exchange/type', [PostalExchangeTypeController::class, 'index'])->name('postal-exchange.type');
        Route::post('postal-exchange/type/store', [PostalExchangeTypeController::class, 'store'])->name('postal-exchange.type.store');
        Route::get('postal-exchange/type/{id}/edit', [PostalExchangeTypeController::class, 'edit'])->name('postal-exchange.type.edit');
        Route::post('postal-exchange/type/{id}/update', [PostalExchangeTypeController::class, 'update'])->name('postal-exchange.type.update');
        Route::get('postal-exchange/type/{id}/delete', [PostalExchangeTypeController::class, 'delete'])->name('postal-exchange.type.delete');
        Route::get('postal-exchange/type/{id}/active', [PostalExchangeTypeController::class, 'Active'])->name('postal-exchange.type.active');
        Route::get('postal-exchange/type/{id}/in-active', [PostalExchangeTypeController::class, 'inActive'])->name('postal-exchange.type.in-active');
        Route::post('postal-exchange/type/bulk-action', [PostalExchangeTypeController::class, 'bulkAction'])->name('postal-exchange.type.bulk-action');
    });
    /* Visitor Log Routes */
    Route::middleware(['ability:super-admin,visitor-index'])->group(function () {
        ...
        Route::post('visitor/{id}/update', [VisitorController::class, 'update'])->name('visitor.update');
        Route::get('visitor/{id}/delete', [VisitorController::class, 'delete'])->name('visitor.delete');
        Route::get('visitor/{id}/active', [VisitorController::class, 'Active'])->name('visitor.active');
        Route::get('visitor/{id}/in-active', [VisitorController::class, 'inActive'])->name('visitor.in-active');
        Route::post('visitor/bulk-action', [VisitorController::class, 'bulkAction'])->name('visitor.bulk-action');
    });
    /* Visitor Purpose Routes */
    Route::middleware(['ability:super-admin,visitor-purpose-index'])->group(function () {
        Route::get('visitor/purpose', [VisitorPurposeController::class, 'index'])->name('visitor.purpose');
        Route::get('visitor/purpose/add', [VisitorPurposeController::class, 'add'])->name('visitor.purpose.add');
        Route::post('visitor/purpose/store', [VisitorPurposeController::class, 'store'])->name('visitor.purpose.store');
        Route::get('visitor/purpose/{id}/edit', [VisitorPurposeController::class, 'edit'])->name('visitor.purpose.edit');
        Route::post('visitor/purpose/{id}/update', [VisitorPurposeController::class, 'update'])->name('visitor.purpose.update');
        Route::get('visitor/purpose/{id}/delete', [VisitorPurposeController::class, 'delete'])->name('visitor.purpose.delete');
        Route::get('visitor/purpose/{id}/active', [VisitorPurposeController::class, 'Active'])->name('visitor.purpose.active');
        Route::get('visitor/purpose/{id}/in-active', [VisitorPurposeController::class, 'inActive'])->name('visitor.purpose.in-active');
        Route::post('visitor/purpose/bulk-action', [VisitorPurposeController::class, 'bulkAction'])->name('visitor.purpose.bulk-action');
    });
});
...
// Public Registration
Route::get('public-registration', [StudentPublicController::class, 'registration'])
    ->name('public-registration.registration');

// Students Guardian Grouping
Route::middleware(['prefix' => 'guardian', 'as' => 'guardian', 'namespace' => 'Student\\'])
    ->group(function () {
        // Guardian List
        Route::middleware(['ability:super-admin,guardian-index'])
            ->get('', [GuardianController::class, 'index'])
            ->name('index');
        // Guardian Registration
        Route::middleware(['ability:super-admin,guardian-registration'])
            ->get('registration', [GuardianController::class, 'registration'])
            ->name('registration');
        Route::middleware(['ability:super-admin,guardian-register'])
            ->post('register', [GuardianController::class, 'register'])
            ->name('register');
        // View Guardian
        Route::middleware(['ability:super-admin,guardian-view'])
            ->get('{id}/view', [GuardianController::class, 'view'])
            ->name('view');
        // Edit Guardian
        Route::middleware(['ability:super-admin,guardian-edit'])
            ->get('{id}/edit', [GuardianController::class, 'edit'])
            ->name('edit');
        Route::middleware(['ability:super-admin,guardian-edit'])
            ->post('{id}/update', [GuardianController::class, 'update'])
            ->name('update');
        // Delete Guardian
        Route::middleware(['ability:super-admin,guardian-delete'])
            ->get('{id}/delete', [GuardianController::class, 'delete'])
            ->name('delete');
        // Activate Guardian
        Route::middleware(['ability:super-admin,guardian-active'])
            ->get('{id}/active', [GuardianController::class, 'Active'])
            ->name('active');
        // Deactivate Guardian
        Route::middleware(['ability:super-admin,guardian-in-active'])
            ->get('{id}/in-active', [GuardianController::class, 'inActive'])
            ->name('in-active');
        // Bulk Action
        Route::middleware(['ability:super-admin,student-bulk-action'])
            ->post('bulk-action', [GuardianController::class, 'bulkAction'])
            ->name('bulk-action');
        // Link Student and Guardian
        Route::middleware(['ability:super-admin,guardian-link'])
            ->post('link', [GuardianController::class, 'link'])
            ->name('link');
        // Unlink Student and Guardian
        Route::middleware(['ability:super-admin,guardian-unlink'])
            ->get('{student}/{guardian}/unlink', [GuardianController::class, 'unlink'])
            ->name('unlink');
});
...
// Staff Grouping
Route::middleware(['prefix' => 'staff', 'as' => 'staff', 'namespace' => 'Staff\\'])
    ->group(function () {
        // Staff Routes
        Route::middleware(['ability:super-admin,staff-index'])
            ->get('', [StaffController::class, 'index'])
            ->name('index');

        Route::middleware(['ability:super-admin,staff-add'])
            ->get('add', [StaffController::class, 'add'])
            ->name('.add');

        Route::middleware(['ability:super-admin,staff-add'])
            ->post('store', [StaffController::class, 'store'])
            ->name('.store');

        Route::middleware(['ability:super-admin,staff-edit'])
            ->get('{id}/edit', [StaffController::class, 'edit'])
            ->name('.edit');

        Route::middleware(['ability:super-admin,staff-edit'])
            ->post('{id}/update', [StaffController::class, 'update'])
            ->name('.update');

        Route::middleware(['ability:super-admin,staff-view'])
            ->get('{id}/view', [StaffController::class, 'view'])
            ->name('.view');

        Route::middleware(['ability:super-admin,staff-delete'])
            ->get('{id}/delete', [StaffController::class, 'delete'])
            ->name('.delete');

        Route::middleware(['ability:super-admin,staff-active'])
            ->get('{id}/active', [StaffController::class, 'Active'])
            ->name('.active');

        Route::middleware(['ability:super-admin,staff-in-active'])
            ->get('{id}/in-active', [StaffController::class, 'inActive'])
            ->name('.in-active');

        Route::middleware(['ability:super-admin,staff-bulk-action'])
            ->post('bulk-action', [StaffController::class, 'bulkAction'])
            ->name('.bulk-action');

        Route::middleware(['ability:super-admin,staff-add'])
            ->get('import', [StaffController::class, 'importStaff'])
            ->name('.import');

        Route::middleware(['ability:super-admin,staff-add'])
            ->post('import', [StaffController::class, 'handleImportStaff'])
            ->name('.bulk.import');

        // Staff login access
        Route::middleware(['ability:super-admin,user-add'])
            ->post('user/create', [StaffController::class, 'createUser'])
            ->name('.user.create');

        Route::middleware(['ability:super-admin,user-edit'])
            ->post('{id}/user/update', [StaffController::class, 'updateUser'])
            ->name('.user.update');

        Route::middleware(['ability:super-admin,user-active'])
            ->get('{id}/user/active', [StaffController::class, 'activeUser'])
            ->name('.user.active');

        Route::middleware(['ability:super-admin,user-in-active'])
            ->get('{id}/user/in-active', [StaffController::class, 'inActiveUser'])
            ->name('.user.in-active');

        Route::middleware(['ability:super-admin,user-delete'])
            ->get('{id}/user/delete', [StaffController::class, 'deleteUser'])
            ->name('.user.delete');

        // Staff Document Upload
        Route::middleware(['ability:super-admin,staff-document-index'])
            ->get('document', [DocumentController::class, 'index'])
            ->name('.document');

        Route::middleware(['ability:super-admin,staff-document-add'])
            ->post('document/store', [DocumentController::class, 'store'])
            ->name('.document.store');

        Route::middleware(['ability:super-admin,staff-document-edit'])
            ->get('document/{id}/edit', [DocumentController::class, 'edit'])
            ->name('.document.edit');

        Route::middleware(['ability:super-admin,staff-document-edit'])
            ->post('document/{id}/update', [DocumentController::class, 'update'])
            ->name('.document.update');

        Route::middleware(['ability:super-admin,staff-document-delete'])
            ->get('document/{id}/delete', [DocumentController::class, 'delete'])
            ->name('.document.delete');

        Route::middleware(['ability:super-admin,staff-document-active'])
            ->get('document/{id}/active', [DocumentController::class, 'Active'])
            ->name('.document.active');

        Route::middleware(['ability:super-admin,staff-document-in-active'])
            ->get('document/{id}/in-active', [DocumentController::class, 'inActive'])
            ->name('.document.in-active');

        Route::middleware(['ability:super-admin,staff-document-bulk-action'])
            ->post('document/bulk-action', [DocumentController::class, 'bulkAction'])
            ->name('.document.bulk-action');

        // Staff Notes Creating
        Route::middleware(['ability:super-admin,staff-note-index'])
            ->get('note', [NoteController::class, 'index'])
            ->name('.note');

        Route::middleware(['ability:super-admin,staff-note-add'])
            ->post('note/store', [NoteController::class, 'store'])
            ->name('.note.store');

        Route::middleware(['ability:super-admin,staff-note-edit'])
            ->get('note/{id}/edit', [NoteController::class, 'edit'])
            ->name('.note.edit');

        Route::middleware(['ability:super-admin,staff-note-edit'])
            ->post('note/{id}/update', [NoteController::class, 'update'])
            ->name('.note.update');

        Route::middleware(['ability:super-admin,staff-note-delete'])
            ->get('note/{id}/delete', [NoteController::class, 'delete'])
            ->name('.note.delete');

        Route::middleware(['ability:super-admin,staff-note-active'])
            ->get('note/{id}/active', [NoteController::class, 'Active'])
            ->name('.note.active');

        Route::middleware(['ability:super-admin,staff-note-in-active'])
            ->get('note/{id}/in-active', [NoteController::class, 'inActive'])
            ->name('.note.in-active');

        Route::middleware(['ability:super-admin,staff-note-bulk-action'])
            ->post('note/bulk-action', [NoteController::class, 'bulkAction'])
            ->name('.note.bulk-action');

        // Staff Designation
        Route::middleware(['ability:super-admin,staff-designation-index'])
            ->get('designation', [DesignationController::class, 'index'])
            ->name('.designation');

        Route::middleware(['ability:super-admin,staff-designation-add'])
            ->post('designation/store', [DesignationController::class, 'store'])
            ->name('.designation.store');

        Route::middleware(['ability:super-admin,staff-designation-edit'])
            ->get('designation/{id}/edit', [DesignationController::class, 'edit'])
            ->name('.designation.edit');

        Route::middleware(['ability:super-admin,staff-designation-edit'])
            ->post('designation/{id}/update', [DesignationController::class, 'update'])
            ->name('.designation.update');

        Route::middleware(['ability:super-admin,staff-designation-delete'])
            ->get('designation/{id}/delete', [DesignationController::class, 'delete'])
            ->name('.designation.delete');

        Route::middleware(['ability:super-admin,staff-designation-active'])
            ->get('designation/{id}/active', [DesignationController::class, 'Active'])
            ->name('.designation.active');

        Route::middleware(['ability:super-admin,staff-designation-in-active'])
            ->get('designation/{id}/in-active', [DesignationController::class, 'inActive'])
            ->name('.designation.in-active');

        Route::middleware(['ability:super-admin,staff-designation-bulk-action'])
            ->post('designation/bulk-action', [DesignationController::class, 'bulkAction'])
            ->name('.designation.bulk-action');
});
...
Route::prefix('account/')
    ...
    ->group(function () {
        // Fees Group
        Route::middleware(['ability:super-admin,fees-index'])
            ->group(function () {
                // Balance Fees
                Route::get('fees/', [FeesBaseController::class, 'index'])->name('fees');
                Route::get('fees/balance', [FeesBaseController::class, 'balance'])->name('fees.balance');
            });

        // Fee Head
        Route::middleware(['ability:super-admin,fees-head-index'])
            ->group(function () {
                Route::get('fees/head', [FeesHeadController::class, 'index'])->name('fees.head');
                Route::post('fees/head/store', [FeesHeadController::class, 'store'])->name('fees.head.store');
                Route::get('fees/head/{id}/edit', [FeesHeadController::class, 'edit'])->name('fees.head.edit');
                Route::post('fees/head/{id}/update', [FeesHeadController::class, 'update'])->name('fees.head.update');
                Route::get('fees/head/{id}/delete', [FeesHeadController::class, 'delete'])->name('fees.head.delete');
                Route::get('fees/head/{id}/active', [FeesHeadController::class, 'Active'])->name('fees.head.active');
                Route::get('fees/head/{id}/in-active', [FeesHeadController::class, 'inActive'])->name('fees.head.in-active');
                Route::post('fees/head/bulk-action', [FeesHeadController::class, 'bulkAction'])->name('fees.head.bulk-action');
                Route::post('fees/head/import', [FeesHeadController::class, 'handleImportFeeHead'])->name('fees.head.bulk.import');
            });
        Route::middleware(['ability:super-admin,fees-master-index'])->group(function () {
            Route::get('fees/master', [FeesMasterController::class, 'index'])->name('fees.master');
            Route::get('fees/master/add', [FeesMasterController::class, 'add'])->name('fees.master.add');
            Route::post('fees/master/store', [FeesMasterController::class, 'store'])->name('fees.master.store');
            Route::get('fees/master/{id}/edit', [FeesMasterController::class, 'edit'])->name('fees.master.edit');
            Route::post('fees/master/{id}/update', [FeesMasterController::class, 'update'])->name('fees.master.update');
            Route::get('fees/master/{id}/delete', [FeesMasterController::class, 'delete'])->name('fees.master.delete');
            Route::post('fees/master/bulk-action', [FeesMasterController::class, 'bulkAction'])->name('fees.master.bulk-action');
            Route::get('fees/master/{id}/active', [FeesMasterController::class, 'Active'])->name('fees.master.active');
            Route::get('fees/master/{id}/in-active', [FeesMasterController::class, 'inActive'])->name('fees.master.in-active');
            Route::post('fees/master/fee-html', [FeesMasterController::class, 'feeHtmlRow'])->name('fees.master.fee-html');
        });
        Route::middleware(['ability:super-admin,fees-quick-receive-add'])->group(function () {
            Route::get('fees/quick-receive', [FeesCollectionController::class, 'quickReceive'])->name('fees.quick-receive');
            Route::post('fees/quick-receive/store', [FeesCollectionController::class, 'quickReceiveStore'])->name('fees.quick-receive.store');
            Route::post('student-detail-html', [FeesCollectionController::class, 'studentDetail'])->name('student-detail-html');
        });
        Route::middleware(['ability:super-admin,fees-collection-view'])->group(function () {
            Route::get('fees/due/{id}/view', [FeesCollectionController::class, 'dueView'])->name('fees.due.view');
            Route::post('fees/due/store', [FeesCollectionController::class, 'dueStore'])->name('fees.due.store');
        });
        Route::middleware(['ability:super-admin,fees-collection-index'])->group(function () {
            Route::get('fees/collection', [FeesCollectionController::class, 'index'])->name('fees.collection');
            Route::get('fees/collection/{id}/add', [FeesCollectionController::class, 'add'])->name('fees.collection.add');
            Route::post('fees/collection/store', [FeesCollectionController::class, 'store'])->name('fees.collection.store');
            Route::get('fees/collection/{id}/view', [FeesCollectionController::class, 'view'])->name('fees.collection.view');
            Route::get('fees/collection/{id}/delete', [FeesCollectionController::class, 'delete'])->name('fees.collection.delete');
        });
        Route::middleware(['ability:super-admin,fees-online-payment-index'])->group(function () {
            Route::get('fees/online-payment', [OnlinePaymentController::class, 'onlinePayment'])->name('fees.online-payment');
            Route::post('fees/online-payment/{id}/verify', [OnlinePaymentController::class, 'paymentVerify'])->name('fees.online-payment.verify');
            Route::get('fees/online-payment/{id}/view', [OnlinePaymentController::class, 'paymentView'])->name('fees.online-payment.view');
        });
    

        Route::name('fees.')->group(function () {
            // PayPal Payment
            Route::get('fees/pay-with-paypal', [PayPalController::class, 'payment'])->name('paypal-form');
            Route::get('fees/paypal-payment-cancel', [PayPalController::class, 'cancel'])->name('paypal-cancel');
            Route::get('fees/paypal-payment-success', [PayPalController::class, 'success'])->name('paypal-success');

            // Stripe Payment
            Route::post('fees/pay-with-stripe', [StripePaymentController::class, 'stripePayment'])->name('stripePayment');

            // Instamojo Payment
            Route::post('fees/pay-with-instamojo/index', [InstamojoPaymentController::class, 'index'])->name('instamojoPayment.index');
            Route::post('fees/pay-with-instamojo/pay', [InstamojoPaymentController::class, 'pay'])->name('instamojoPayment.pay');
            Route::get('fees/pay-with-instamojo/pay-success', [InstamojoPaymentController::class, 'success'])->name('instamojoPayment.success');

            // Payumoney Payment
            Route::post('fees/payumoney-form', [PayumoneyPaymentController::class, 'payumoneyForm'])->name('payumoney-form');
            Route::post('fees/pay-with-payumoney/success', [PayumoneyPaymentController::class, 'payumoneyPaymentSuccess'])->name('payumoney.success');
            Route::post('fees/pay-with-payumoney/failure', [PayumoneyPaymentController::class, 'payumoneyPaymentFailure'])->name('payumoney.failure');

            // PayStack Payment
            Route::post('fees/pay-stack-form', [PayStackPaymentController::class, 'redirectToGateway'])->name('pay-stack-form');
            Route::get('fees/pay-with-pay-stack/success', [PayStackPaymentController::class, 'handleGatewayCallback'])->name('pay-stack.success');

            // Razorpay Payment
            Route::get('fees/pay-with-razorpay-form', [RozorPayPaymentController::class, 'paywithrazorpay'])->name('pay-with-razorpay-form');
            Route::get('fees/pay-with-razorpay/success', [RozorPayPaymentController::class, 'payment'])->name('pay-with-razorpay.success');
        });

       // Payroll Routes
        Route::middleware(['ability:super-admin,payroll-index'])->group(function () {
            Route::get('payroll/', [PayrollBaseController::class, 'index'])->name('payroll');
            Route::get('payroll/balance', [PayrollBaseController::class, 'balance'])->name('payroll.balance');
        });
        
        Route::name('payroll.')->group(function () {
            // Payroll Head Routes
            Route::middleware(['ability:super-admin,payroll-head-index'])->group(function () {
                Route::get('payroll/head', [PayrollHeadController::class, 'index'])->name('head');
                Route::post('payroll/head/store', [PayrollHeadController::class, 'store'])->name('head.store');
                Route::get('payroll/head/{id}/edit', [PayrollHeadController::class, 'edit'])->name('head.edit');
                Route::post('payroll/head/{id}/update', [PayrollHeadController::class, 'update'])->name('head.update');
                Route::get('payroll/head/{id}/delete', [PayrollHeadController::class, 'delete'])->name('head.delete');
                Route::get('payroll/head/{id}/active', [PayrollHeadController::class, 'Active'])->name('head.active');
                Route::get('payroll/head/{id}/in-active', [PayrollHeadController::class, 'inActive'])->name('head.in-active');
                Route::post('payroll/head/bulk-action', [PayrollHeadController::class, 'bulkAction'])->name('head.bulk-action');
            });
        
            // Payroll Master Routes
            Route::middleware(['ability:super-admin,payroll-master-index'])->group(function () {
                Route::get('payroll/master', [PayrollMasterController::class, 'index'])->name('master');
                Route::get('payroll/master/add', [PayrollMasterController::class, 'add'])->name('master.add');
                Route::post('payroll/master/store', [PayrollMasterController::class, 'store'])->name('master.store');
                Route::get('payroll/master/{id}/edit', [PayrollMasterController::class, 'edit'])->name('master.edit');
                Route::post('payroll/master/{id}/update', [PayrollMasterController::class, 'update'])->name('master.update');
                Route::get('payroll/master/{id}/delete', [PayrollMasterController::class, 'delete'])->name('master.delete');
                Route::post('payroll/master/bulk-action', [PayrollMasterController::class, 'bulkAction'])->name('master.bulk-action');
                Route::get('payroll/master/{id}/active', [PayrollMasterController::class, 'Active'])->name('master.active');
                Route::get('payroll/master/{id}/in-active', [PayrollMasterController::class, 'inActive'])->name('master.in-active');
                Route::post('payroll/master/payroll-html', [PayrollMasterController::class, 'payrollHtmlRow'])->name('master.payroll-html');
            });
        });
        ...
        Route::name('salary.')->group(function () {
            ...
            Route::middleware(['ability:super-admin,salary-payment-index'])->group(function () {
                Route::get('salary/payment', [SalaryPayController::class, 'index'])->name('payment');
                Route::get('salary/payment/{id}/add', [SalaryPayController::class, 'add'])->name('payment.add');
                Route::post('salary/payment/store', [SalaryPayController::class, 'store'])->name('payment.store');
                Route::get('salary/payment/{id}/view', [SalaryPayController::class, 'view'])->name('payment.view');
                Route::get('salary/payment/{id}/delete', [SalaryPayController::class, 'delete'])->name('payment.delete');
            });
        });

        Route::name('transaction.')->group(function () {
            // Account Group Routes
            Route::middleware(['ability:super-admin,account-group-index'])->group(function () {
                Route::get('account-group', [AccountGroupController::class, 'index'])->name('account-group');
                Route::post('account-group/store', [AccountGroupController::class, 'store'])->name('account-group.store');
                Route::get('account-group/{id}/edit', [AccountGroupController::class, 'edit'])->name('account-group.edit');
                Route::post('account-group/{id}/update', [AccountGroupController::class, 'update'])->name('account-group.update');
                Route::get('account-group/{id}/delete', [AccountGroupController::class, 'delete'])->name('account-group.delete');
                Route::get('account-group/{id}/active', [AccountGroupController::class, 'Active'])->name('account-group.active');
                Route::get('account-group/{id}/in-active', [AccountGroupController::class, 'inActive'])->name('account-group.in-active');
                Route::post('account-group/bulk-action', [AccountGroupController::class, 'bulkAction'])->name('account-group.bulk-action');
                Route::get('account-group/chart-of-account', [AccountGroupController::class, 'chartOfAccount'])->name('account-group.chart-of-account');
            });
        });
       
        
        Route::name('transaction-head.')->middleware(['ability:super-admin,transaction-head-index'])->group(function () {
            Route::get('transaction-head', [TransactionHeadController::class, 'index'])->name('index');
            Route::post('transaction-head/store', [TransactionHeadController::class, 'store'])->name('store');
            Route::get('transaction-head/{id}/edit', [TransactionHeadController::class, 'edit'])->name('edit');
            Route::post('transaction-head/{id}/update', [TransactionHeadController::class, 'update'])->name('update');
            Route::get('transaction-head/{id}/delete', [TransactionHeadController::class, 'delete'])->name('delete');
            Route::get('transaction-head/{id}/active', [TransactionHeadController::class, 'Active'])->name('active');
            Route::get('transaction-head/{id}/in-active', [TransactionHeadController::class, 'inActive'])->name('in-active');
            Route::post('transaction-head/bulk-action', [TransactionHeadController::class, 'bulkAction'])->name('bulk-action');
            Route::get('transaction-head/view', [TransactionHeadController::class, 'view'])->name('view');
            Route::get('transaction-head/balance-statement', [TransactionHeadController::class, 'viewBalanceStatement'])->name('balance-statement');
        });
        Route::name('transaction.')->middleware(['ability:super-admin,transaction-index'])->group(function () {
            Route::get('transaction', [TransactionController::class, 'index'])->name('index');
            Route::get('transaction/add', [TransactionController::class, 'add'])->name('add');
            Route::post('transaction/store', [TransactionController::class, 'store'])->name('store');
            Route::get('transaction/{id}/delete', [TransactionController::class, 'delete'])->name('delete');
            Route::post('transaction/bulk-action', [TransactionController::class, 'bulkAction'])->name('bulk-action');
            // Multi Transaction
            Route::get('transaction/multi-add', [TransactionController::class, 'multiAdd'])->name('multi-add');
            Route::post('transaction/multi-store', [TransactionController::class, 'multiStore'])->name('multi-store');
            Route::post('transaction/tr-html', [TransactionController::class, 'trHtmlRow'])->name('tr-html');

            // Ac to Ac Transfer
            Route::get('transfer', [TransferController::class, 'actransfer'])->name('actransfer');
            Route::post('transfer/store', [TransferController::class, 'transferstore'])->name('transferstore');
        });
        // bank 
        Route::name('bank.')->middleware(['ability:super-admin,bank-index'])->group(function () {
            Route::get('bank', [BankController::class, 'index'])->name('index');
            Route::get('bank/add', [BankController::class, 'add'])->name('add');
            Route::post('bank/store', [BankController::class, 'store'])->name('store');
            Route::get('bank/{id}/edit', [BankController::class, 'edit'])->name('edit');
            Route::post('bank/{id}/update', [BankController::class, 'update'])->name('update');
            Route::get('bank/{id}/view', [BankController::class, 'view'])->name('view');
            Route::get('bank/{id}/delete', [BankController::class, 'delete'])->name('delete');
            Route::post('bank/bulk-action', [BankController::class, 'bulkAction'])->name('bulk-action');
            Route::get('bank/{id}/active', [BankController::class, 'Active'])->name('active');
            Route::get('bank/{id}/in-active', [BankController::class, 'inActive'])->name('in-active');
        });
        Route::middleware(['ability:super-admin'])->group(function () {
            // Bank Transaction Routes
            Route::name('bank-transaction.')->group(function () {
                Route::get('bank-transaction', [BankTransactionController::class, 'index'])->name('index');
                Route::get('bank-transaction/add', [BankTransactionController::class, 'add'])->name('add');
                Route::post('bank-transaction/store', [BankTransactionController::class, 'store'])->name('store');
                Route::get('bank-transaction/{id}/delete', [BankTransactionController::class, 'delete'])->name('delete');
                Route::post('bank-transaction/bulk-action', [BankTransactionController::class, 'bulkAction'])->name('bulk-action');
            });

            // Account Report Routes
            Route::name('report.')->group(function () {
                Route::get('report/cash-book', [CashBookReportController::class, 'cashBook'])->name('cash-book');
                Route::get('report/fee-collection', [FeeCollectionReportController::class, 'feeCollection'])->name('fee-collection');
                Route::get('report/fee-collection-head', [FeeCollectionHeadReportController::class, 'feeCollectionHead'])->name('fee-collection-head');
                Route::get('report/fee-online-payment', [OnlineFeePaymentReportController::class, 'onlinePayments'])->name('fee-online-payment');
                Route::get('report/balance-fee', [BalanceFeeReportController::class, 'balanceFees'])->name('balance-fee');
            });
        });
        // ... Other routes for Fees and Accounting Group
 });
...
Route::middleware(['ability:super-admin'])->prefix('library')->name('library.')->namespace('Library\\')->group(function () {
    Route::get('', [LibraryBaseController::class, 'index'])->name('index');
    Route::post('issue', [LibraryBaseController::class, 'issueBook'])->name('issue');
    Route::get('{id}/{member}/request-cancel', [LibraryBaseController::class, 'bookRequestCancel'])->name('request-cancel');
    Route::get('{id}/return', [LibraryBaseController::class, 'returnBook'])->middleware('ability:super-admin,library-book-return')->name('return');
    Route::get('return-over', [LibraryBaseController::class, 'returnOver'])->middleware('ability:super-admin,library-return-over')->name('return-over');
    Route::get('issue-history', [LibraryBaseController::class, 'issueHistory'])->middleware('ability:super-admin,library-issue-history')->name('issue-history');
    Route::post('book-detail-html', [LibraryBaseController::class, 'bookDetail'])->name('book-detail-html');
    Route::get('book-name-autocomplete', [LibraryBaseController::class, 'bookNameAutocomplete'])->name('book-name-autocomplete');
    ...
    Route::get('book/import', [BookController::class, 'importBook'])->name('book.import');
    Route::post('book/import', [BookController::class, 'handleImportBook'])->name('book.bulk.import');
    Route::get('book/{id}/view', [BookController::class, 'view'])->name('book.view');
    Route::get('book/{id}/delete', [BookController::class, 'delete'])->name('book.delete');
    Route::get('book/{id}/active', [BookController::class, 'Active'])->name('book.active');
    Route::get('book/{id}/in-active', [BookController::class, 'inActive'])->name('book.in-active');
    Route::post('book/bulk-action', [BookController::class, 'bulkAction'])->name('book.bulk-action');
    Route::post('book/add/copies', [BookController::class, 'addCopies'])->name('book.add.copies');
    Route::get('book/{id}/book-status/{status}', [BookController::class, 'bookStatus'])->name('book.book-status');
    Route::post('book/bulk-copies-delete', [BookController::class, 'bulkCopiesDelete'])->name('book.bulk-copies-delete');
